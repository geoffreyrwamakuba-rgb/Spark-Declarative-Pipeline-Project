...
# Configuration values are read from pipeline settings inside the function
...
def calendar():
    # Read configuration values inside the function (not at module level)
    start_date = spark.conf.get("start_date")
    end_date = spark.conf.get("end_date")
    
    df = (spark.sql(f"""SELECT explode(sequence(
        to_date('{start_date}'),
        to_date('{end_date}'),interval 1 day)) as date"""))
    # create key that removes - from the dates
